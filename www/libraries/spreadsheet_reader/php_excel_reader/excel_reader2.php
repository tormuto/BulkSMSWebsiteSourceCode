...
class OLERead {
	...
	function read($sFileName){
		...
		if (!$this->data) {
			$this->error = 1;
			$this->error_msg='The filename ' . $sFileName . ' could not be fetched.';
			return false;
   		}
   		if (substr($this->data, 0, 8) != IDENTIFIER_OLE) {
			$this->error = 1;
			$this->error_msg='The file ' . $sFileName . ' is corrupted or not a valid .xls file. Please try re-saving the file using a proper programme.';
			return false;
   		}
	...
	function read($sFileName) {
		...
		if($res === false) {
			...
			if($this->_ole->error == 1) {
				// bad file
				if(!empty($this->_ole->error_msg))$msg=$this->_ole->error_msg;
				else $msg='The filename ' . $sFileName . ' is not readable';
