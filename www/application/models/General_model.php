<?php
	class General_model extends CI_Model
	{
		public function __construct(){
			...
			if(
				defined('_SMTP_HOST_')&&!empty(_SMTP_HOST_)&&
				defined('_SMTP_PORT_')&&!empty(_SMTP_PORT_)&&
				defined('_SMTP_USER_')&&!empty(_SMTP_USER_)&&
				defined('_SMTP_PASS_')&&!empty(_SMTP_PASS_)
			){
				$this->smtp_settings=array(
					'protocol'  => 'smtp',
					'smtp_host' => _SMTP_HOST_,
					'smtp_port' => _SMTP_PORT_,
					'smtp_user' => _SMTP_USER_,
					'smtp_pass' => _SMTP_PASS_,
				);
			}
...
*/
		...
		public function admin_logged_in(){ return ($this->session->userdata('admin_logged_in')==1); }
	...
	###################################  CURRENCIES #################
		...
		function get_sms_log($only_count=false,$filter=''){
			...
			$this->db->join('contacts',"contacts.phone=sms_log.recipient AND "._DB_PREFIX_."contacts.sub_account_id="._DB_PREFIX_."sms_log.sub_account_id AND "._DB_PREFIX_."contacts.user_id="._DB_PREFIX_."sms_log.user_id AND ("._DB_PREFIX_."contacts.firstname!='' OR "._DB_PREFIX_."contacts.lastname!='') ",'left')->group_by('sms_log.sms_id');
		...
		function send_email($to,$subject,$message,$extras="",$attachments=null)
		{
			...
			if(defined(_SMTP_USER_))$from=_SMTP_USER_;
			else $from="no_reply@$domain";
			
			if(is_array($extras)&&!empty($extras)){
				if(!empty($extras['from_name']))$from_name=$extras['from_name'];
				if(!empty($extras['from'])){
					$from=$extras['from'];
					if(empty($extras['from_name']))$from_name='';
				}
			}
			...
			if(defined('_SMTP_HOST_')&&defined('_SMTP_USER_')&&defined('_SMTP_PASS_')&&defined('_SMTP_PORT_')){ 
				/*To use gmail:
				Give google access permissions https://support.google.com/accounts/answer/6010255?hl=en
				...
				*/
				...
				if(empty($extras['reply_to'])&&!empty($extras['from']))$extras['reply_to']=$extras['from'];
				$from=_SMTP_USER_;
			}
			else {
		...
		function get_domain(){
			return (substr($_SERVER['HTTP_HOST'],0,4)=='www.')?substr($_SERVER['HTTP_HOST'],4):$_SERVER['HTTP_HOST'];
		}
